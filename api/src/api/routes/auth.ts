...
export const initAuth = (app: express.Express) => {
    ...
    app.post("/auth/login", async (req: express.Request, res: express.Response) => {
        try {
            ...
            const validPassword = await bcrypt.compare(loginRequest.password, existingUser.password)
            if (!validPassword) {
                return res.status(403).send({error: "Invalid login credentials"})
            }
    ...
    app.post("/auth/refreshToken", async (req: express.Request, res: express.Response) => {
        try {
            ...
            const payload: any = jwt.verify(refreshTokenRequest.token, process.env.JWT_REFRESH_SECRET!)
    ...
    app.post("/auth/revokeRefreshToken", async (req: express.Request, res: express.Response) => {
        try {
            ...
            return res.json({message: `Refresh token revoked for user with id #${revokeRefreshTokenRequest.userId}`})
        } catch (error) {
            return res.status(500).send({error: error})
        }
    })

    app.get("/auth/check", authMiddleware, authzMiddleware(["ADMIN", "SUPER_ADMIN"]), async (req: express.Request, res: express.Response) => {
        try {
